...
TSharedRef<SEditableTextBox> GAPIKeyBlock = SNew(SEditableTextBox)
												.Text(FText::FromString(FString(UTF8_TO_TCHAR(GAPIKey.c_str()))))
												.MinDesiredWidth(500);
...
void FWakaTimeForUEModule::StartupModule()
{
	...
	GBaseCommand = "/c start \"\" /b \"" + string(GUserProfile) + "\\.wakatime\\" + GWakaCliVersion + "\"";

	FString gbc = GBaseCommand.c_str();
	UE_LOG(LogWakaTime, Log, TEXT("Am facut si eu o schema: %s"), *gbc);
...
void FWakaTimeForUEModule::DownloadWakatimeCli(string CliPath)
{
	...
	if (bSuccessDownload)
	{
		...
		if (bSuccessUnzip)
			UE_LOG(LogWakaTime, Log, TEXT("Successfully extracted wakatime-cli."));
	}
	else
	{
...
void FWakaTimeForUEModule::OpenSettingsWindow()
{
	SettingsWindow = SNew(SWindow)
						 ...
						 .IsTopmostWindow(true)
							 [SNew(SVerticalBox) + SVerticalBox::Slot().HAlign(HAlign_Center).VAlign(VAlign_Center)[SNew(SVerticalBox) + SVerticalBox::Slot().HAlign(HAlign_Left).VAlign(VAlign_Top)[SNew(STextBlock).Text(FText::FromString(TEXT("Your api key:"))).MinDesiredWidth(500)] + SVerticalBox::Slot().HAlign(HAlign_Center).VAlign(VAlign_Center)[GAPIKeyBlock]] + SVerticalBox::Slot().HAlign(HAlign_Center).VAlign(VAlign_Bottom)[SNew(SBox).WidthOverride(100)[SNew(SButton).Text(FText::FromString(TEXT("Save"))).OnClicked(FOnClicked::CreateRaw(this, &FWakaTimeForUEModule::SaveData))]]];
	IMainFrameModule &MainFrameModule =
		FModuleManager::LoadModuleChecked<IMainFrameModule>(TEXT("MainFrame"));
	if (MainFrameModule.GetParentWindow().IsValid())
	{
		FSlateApplication::Get().AddWindowAsNativeChild(SettingsWindow, MainFrameModule.GetParentWindow()
																			.ToSharedRef());
	}
	else
	{
